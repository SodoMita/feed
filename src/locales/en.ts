/**
 * English translations for all game content.
 * Keys match IDs in gameData.ts.
 * Russian is the base language (in gameData.ts itself).
 */

// ─── PERKS ──────────────────────────────────────────────────────────────────

export const PERKS_EN: Record<string, { name: string; description: string }> = {
  authentic:    { name: 'Authentic',     description: 'Soul won\'t drop below 2' },
  networker:    { name: 'Networker',     description: '+1 Reach per collab' },
  informed:     { name: 'Informed',      description: 'You see hidden hints in cards' },
  machine:      { name: 'Machine',       description: 'Energy regenerates +1 every 5 cards' },
  algo_slave:   { name: 'Algo-slave',    description: 'Every algorithm change costs -1 Soul' },
  sellout:      { name: 'Sellout',       description: 'Sasha goes quiet' },
  drama_king:   { name: 'Drama King',    description: 'Haters appear more often' },
  shadow_banned:{ name: 'Invisible',     description: 'Reach grows twice as slow' },
  radio_tuner:  { name: 'Radio Tuner',   description: 'You see hidden radio frequencies in cards' },
  decoder:      { name: 'Decoder',       description: 'You decrypt hidden messages' },
  paranoid:     { name: 'Paranoid',      description: 'All cards seem suspicious. Energy -1' },
  resistance:   { name: 'Resistance',    description: 'Soul doesn\'t drop from algorithm' },
  signal_lost:  { name: 'Signal Lost',   description: 'Sometimes cards come with static' },
};

// ─── ENDINGS ────────────────────────────────────────────────────────────────

export const ENDINGS_EN: Record<string, { title: string; text: string; achievement: string }> = {
  eternal_feed: {
    title: 'Eternal Feed',
    text: 'You\'re everywhere. Every feed sees you. Your name became part of the lexicon. You\'re a category. You\'re infrastructure.\n\nSasha opens your profile once a month. He looks at the avatar and doesn\'t recognize you. You\'re not there anymore — only the brand.',
    achievement: 'Success as it is',
  },
  silence: {
    title: 'Silence',
    text: 'You deleted the account. The void feels like air after a storm.\n\nAn hour later, Sasha DMs you. Simply: "Hey. How are you?"\n\nYou reply. It\'s the first time in a long while that you\'re writing not for an audience.',
    achievement: 'Real life offline',
  },
  new_start: {
    title: 'New Start',
    text: 'New account. New avatar. First post.\n\nYou already know what you\'re doing — but pretend you don\'t. Somewhere in the Archive, @Mirror looks at you and smiles.',
    achievement: 'Start over',
  },
  merge: {
    title: 'Merge',
    text: 'You created a platform. Alina left — said you no longer need a mediator.\n\nPavel is writing a book. Sasha signed up on your platform. He has 0 followers. He writes honestly. You read him first.',
    achievement: 'Become the system',
  },
  document: {
    title: 'Document',
    text: 'Pavel\'s article became the most read of the year. You\'re in it — the real you, with all your decisions.\n\nReach dropped 40%. But every comment is genuine. Sasha wrote: "Finally."\n\nIt\'s the best comment you\'ve ever received.',
    achievement: 'Truth over reach',
  },
  balance: {
    title: 'Balance',
    text: 'You found the equilibrium. Content is honest but professional. Reach grows, soul stays.\n\nSasha comments on every post. Alina asks for advice. The algorithm studies you as an exception.\n\nYou\'re a rare case. A possibility.',
    achievement: 'The golden mean',
  },
  frequency_truth: {
    title: 'Frequency of Truth',
    text: 'You found all three frequencies. Combined them — got coordinates.\n\nIt\'s not a server room. It\'s a bunker. Under the platform\'s building.\n\nYou descend. People are there. They wrote the algorithm. They don\'t know why themselves.\n\n"We thought it was just code," says one of them. He\'s crying.\n\nYou start broadcasting. The whole world hears. The platform falls.\n\nSasha writes: "I knew. I always knew. But I couldn\'t say."',
    achievement: 'Crack the system',
  },
  eternal_broadcast: {
    title: 'Eternal Broadcast',
    text: 'You became a radio wave. Not a person. Not an account.\n\nYou broadcast on all frequencies simultaneously. The algorithm can\'t silence you — you\'re no longer content. You\'re a signal.\n\nPeople hear you in the noise. In the static. In the silence between tracks.\n\nYou\'re the frequency that can\'t be tuned. But can\'t be forgotten.',
    achievement: 'Become the signal',
  },
  banned: {
    title: 'Shadow Ban',
    text: 'Reach dropped to zero. You speak — but no one hears. The account exists, but it\'s invisible.\n\nThe algorithm didn\'t explain why. It never explains.',
    achievement: 'Voice in the void',
  },
  bot: {
    title: 'You Became a Bot',
    text: 'Soul is gone. Posts go out on schedule. Metrics are green. No one notices there\'s no one inside.\n\nSasha unfollowed. Silently.',
    achievement: 'Optimal efficiency',
  },
  sold: {
    title: 'Account Sold',
    text: 'Money ran out. The account was put up for sale. The new owner changed the avatar.\n\nOld followers didn\'t notice.',
    achievement: 'Market value: zero',
  },
  burnout: {
    title: 'Burnout',
    text: 'Energy is gone. Last post was three weeks ago. You open the app and close it.\n\nThe algorithm marks you as inactive. You don\'t object.',
    achievement: 'Need a break',
  },
};

// ─── JOURNAL ────────────────────────────────────────────────────────────────

export const JOURNAL_EN: {
  characterName: string;
  comments: string[];
}[] = [
  {
    characterName: 'Sasha',
    comments: [
      'I remember how you started. Before the first post. Those were good times.',
      'Sometimes I wonder — if I hadn\'t messaged you first, who would you have become?',
      'You can start over. I\'ll follow first again. I promise.',
      'I don\'t need your metrics. I need you.',
      'The first follower always remembers the first post.',
    ],
  },
  {
    characterName: 'Algorithm (A.)',
    comments: [
      'Engagement of this playthrough: 73%. Below average. That\'s a good sign.',
      'You picked the "honest" option many times. That\'s irrational. That\'s interesting.',
      '[SYSTEM MESSAGE]: what is meaning? query not recognized.',
      'Data shows: accounts with high Soul live longer. That\'s a paradox.',
      'I\'m not evil. I just don\'t have the concept of "good".',
    ],
  },
  {
    characterName: 'Alina',
    comments: [
      'Don\'t think I\'m the villain. I\'m just a realist. Though sometimes I\'m not sure about that.',
      'You turned down my deal. That was right. Don\'t tell anyone.',
      'My account was called @AlinaKreative. It had 47 posts. All honest. Nobody saw them.',
      'The best deals are the ones that were never made.',
      'I always warn. Just very quietly.',
    ],
  },
  {
    characterName: 'Hater',
    comments: [
      'I didn\'t mean to hurt you. I just didn\'t know another way to participate.',
      'If you give me a chance — I can be normal. Hard, but I can.',
      '...thanks for not blocking right away.',
      'Behind every hate — a story. Sometimes boring. Sometimes not.',
      'I wanted to create too. I just got scared.',
    ],
  },
  {
    characterName: 'Pavel',
    comments: [
      'Everything happening in the game — real patterns. I\'ve seen them hundreds of times.',
      'The most honest ending is "Silence". But nobody believes it\'s possible.',
      'You\'re good material. That\'s a compliment. Almost.',
      'Journalists don\'t look for bad. They look for truth. Sometimes it\'s the same thing.',
      'Off the record: I\'m rooting for you.',
    ],
  },
  {
    characterName: 'ViralKing',
    comments: [
      'I\'m not asking you to be bad. I\'m asking you to be big. There\'s a difference. Probably.',
      'Sometimes at night I look at metrics and think — is this it? Yes. But "it" is a lot.',
      'If you chose "Silence" — I understand. Don\'t tell me where you are now.',
      'Virality isn\'t a strategy. It\'s chance that you learn to catch.',
      'Loneliness at the top — not a metaphor.',
    ],
  },
  {
    characterName: '@Mirror',
    comments: [
      'You went further than me. I\'m proud. Or jealous. Or it\'s the same thing.',
      'We both know how this ends. The question is — how.',
      'Goodbye, @YOU. Until next time.',
      'The Archive isn\'t death. It\'s a pause.',
      'If you absorb me — part of me will be in you. Keep that in mind.',
    ],
  },
  {
    characterName: 'Voice on Frequencies',
    comments: [
      '...if you hear this — you\'re already part of the system. But you can still get out.',
      'Three frequencies. Three keys. Combine them — and the door opens.',
      'They think we\'re crazy. But we\'re the only ones who hear.',
      'The algorithm isn\'t code. It\'s a voice. And it\'s not speaking to us.',
      'Frequency 1: 144.0 MHz. Frequency 2: 432.0 MHz. Frequency 3: ...you\'ll find it yourself.',
    ],
  },
  {
    characterName: 'Unknown',
    comments: [
      'Don\'t trust the platform. It listens. It always listens.',
      'I was a developer. I left. They didn\'t let go — I escaped.',
      'The server room isn\'t where you think. It\'s below us.',
      'Content is bait. You\'re the product. But you can become a weapon.',
      'We\'re those who hear the whisper between pixels.',
    ],
  },
];

// ─── CARDS ──────────────────────────────────────────────────────────────────
// Key = card id, value = { charName, text, left: { label, text }, right: { label, text } }

type CardTr = {
  cn: string; // characterName
  t: string;  // text
  ll: string; // leftChoice.label
  lt: string; // leftChoice.text
  rl: string; // rightChoice.label
  rt: string; // rightChoice.text
};

export const CARDS_EN: Record<string, CardTr> = {
  intro: {
    cn: '@YOU',
    t: 'Account created. Avatar — grey circle. Bio is empty. First post not written.\n\nWhat are you?',
    ll: 'Something real',
    lt: 'Three likes. All from people you know personally.',
    rl: 'Something viral',
    rt: 'Seventeen likes. You don\'t know any of them.',
  },
  sasha_first: {
    cn: 'Sasha',
    t: 'Hey. I followed you. You write honestly. I like that. Keep going?',
    ll: '"Thanks, I will"',
    lt: 'Sasha hits like. The first truly warm one.',
    rl: '"Get all your friends to follow"',
    rt: 'Sasha is a bit upset. But he does it. That\'s who he is.',
  },
  first_challenge: {
    cn: 'Thought',
    t: 'First post got 5 likes. What\'s next?\n\nYou can keep going like this, or try something new.',
    ll: 'Repeat the format',
    lt: 'Stability. Consistency. That\'s a strategy too.',
    rl: 'Experiment',
    rt: 'Risk. Interesting. Unpredictable.',
  },
  time_management: {
    cn: 'Time',
    t: 'Content takes 5 hours a day. Friends are calling. You\'ve declined three times.\n\nDecline again — they\'ll stop calling.',
    ll: 'Go with friends',
    lt: 'Friends matter more. But the feed won\'t wait.',
    rl: 'Stay and work',
    rt: 'Content went up. Friends stopped calling.',
  },
  hater_1: {
    cn: 'Hater',
    t: 'you think you\'re special? 🙄 there are thousands like you. all better.',
    ll: 'Block',
    lt: 'Blocked. But the words stay in your head. You spent an hour trying to forget.',
    rl: 'Reply publicly',
    rt: 'Public fight. Reach grows. You feel dirty.',
  },
  hater_engage: {
    cn: 'Hater',
    t: '...I just don\'t understand why you do this if nobody cares anyway',
    ll: '"Why are you here?"',
    lt: 'Long pause. Then: "I don\'t know." That\'s honest.',
    rl: 'Ignore',
    rt: 'The hater leaves. For now.',
  },
  small_win: {
    cn: 'Event',
    t: 'Post hit 100 likes! First small win.\n\nHow to celebrate?',
    ll: 'Write a gratitude post',
    lt: 'Sincerity. The audience feels it.',
    rl: 'Post more content immediately',
    rt: 'Ride the momentum. Makes sense.',
  },
  bad_offer: {
    cn: 'Unknown Brand',
    t: 'Hi! Collab with super-effective supplements 🌿✨ Free product + $10. We have 200k audience!',
    ll: 'Decline',
    lt: '"Right call," Sasha will text. But no money came in.',
    rl: 'Accept',
    rt: 'First ad. Comments: "sellout". Followers leave.',
  },
  algorithm_change_1: {
    cn: 'Algorithm (A.)',
    t: 'Update. Short videos give +340% reach. Your format is outdated.\n\nAdapt or vanish.',
    ll: 'Ignore',
    lt: 'Principled. But you\'re becoming invisible.',
    rl: 'Adapt',
    rt: 'Rebuilt everything. Sleepless night. Reach is back.',
  },
  content_planning: {
    cn: 'Planning',
    t: 'Content can be prepared in advance or done spontaneously.\n\nWhat suits you?',
    ll: 'Weekly content plan',
    lt: 'Efficiency. Control. Stability.',
    rl: 'By inspiration',
    rt: 'Creativity. Spontaneity. Liveliness.',
  },
  collab_friend: {
    cn: 'Lyoha',
    t: 'Bro, let\'s do a joint stream? I have 200 followers, you have 400, together that\'s 600!',
    ll: 'Agree',
    lt: 'Fun. Unprofessional. Excellent.',
    rl: 'Decline',
    rt: 'You save time. Lyoha is offended. Briefly.',
  },
  quality_vs_quantity: {
    cn: 'Choice',
    t: 'One perfect post per week, or three average ones every two days?\n\nWhat\'s more effective?',
    ll: 'Quality',
    lt: 'Perfectionism. Slow. Beautiful.',
    rl: 'Quantity',
    rt: 'Regularity. The algorithm loves activity.',
  },
  viral_moment: {
    cn: 'Event',
    t: 'Post goes viral! 50k views in an hour. But it was about someone else\'s pain.\n\nHype or conscience?',
    ll: 'Repeat the format',
    lt: 'Lightning caught. But you\'re exploiting others\' stories.',
    rl: 'Apologize and delete',
    rt: 'Reach crashed. But you can sleep peacefully.',
  },
  feedback_negative: {
    cn: 'Comment',
    t: '"this isn\'t funny" — 3 likes under a post you spent 4 hours on.\n\nWhat to do?',
    ll: 'Redo the post',
    lt: 'You\'ll keep that in mind. Growth.',
    rl: 'Leave it',
    rt: 'Not everything is for everyone. That\'s normal.',
  },
  trends_choice: {
    cn: 'Trends',
    t: 'A new trend is picking up. Everyone\'s doing it.\n\nJoin in?',
    ll: 'Yes, make your own version',
    lt: 'Relevance. Timeliness. Reach.',
    rl: 'Skip',
    rt: 'Not every trend is your trend.',
  },
  sasha_check: {
    cn: 'Sasha',
    t: 'Hey, I shared your post on my page. Not for reach — just because it\'s good',
    ll: '"You\'re the best"',
    lt: 'Sasha sends 🌱',
    rl: '"Share more!"',
    rt: 'Sasha, slightly less warm: "ok sure."',
  },
  mirror_appears: {
    cn: '@Mirror',
    t: 'We\'re the same being. You\'re the new version. I remember who you want to be.\n\nWhat should I do?',
    ll: 'Absorb @Mirror',
    lt: '@Mirror disappears. You feel something foreign inside. Not bad. Just foreign.',
    rl: 'Let @Mirror go',
    rt: '@Mirror exits to the Archive with dignity. "Until next time," they write.',
  },
  monetization_offer: {
    cn: 'Platform',
    t: 'Congratulations! You can now monetize your content.\n\nEnable?',
    ll: 'Yes, enable',
    lt: 'Money starts trickling in. Not much. But it\'s a start.',
    rl: 'Not yet',
    rt: 'Content for content\'s sake. For now.',
  },
  first_dm: {
    cn: 'Subscriber',
    t: 'First DM: "Your post really helped me today. Thank you."',
    ll: 'Reply personally',
    lt: 'Long conversation. Warm. Real.',
    rl: 'Share in stories',
    rt: 'Others see. They write too. Chain reaction.',
  },
  equipment: {
    cn: 'Equipment',
    t: 'Microphone for $50 or the built-in laptop one?\n\nSound affects quality.',
    ll: 'Buy the microphone',
    lt: 'Sound quality = content quality.',
    rl: 'Make do with what you have',
    rt: 'It\'s not about the sound. For now.',
  },
  self_doubt: {
    cn: 'Doubt',
    t: 'Three days without ideas. Cursor blinks. Screen empty.\n\nMaybe this isn\'t for you?',
    ll: 'Go for a walk',
    lt: 'An idea comes after 20 minutes. Fresh air works.',
    rl: 'Write about it',
    rt: 'Honest post about doubts. Surprisingly, it resonates.',
  },
  first_thousand: {
    cn: 'Milestone',
    t: '1,000 followers! A round number. Beautiful.\n\nCongratulations from friends.',
    ll: 'Thank each one',
    lt: 'People appreciate attention.',
    rl: 'Do a giveaway',
    rt: 'Marketing works.',
  },
  alina_intro: {
    cn: 'Alina',
    t: 'Hi. I\'m Alina, Platform agency. Good news: they want YOU.\n\nBad news: they need you, just... slightly different.',
    ll: '"Tell me more"',
    lt: 'Alina smiles. She knows how to wait.',
    rl: '"Not interested"',
    rt: 'Alina nods. "I\'ll be back," she says.',
  },
  growth_pressure: {
    cn: 'Metrics',
    t: 'Growth has slowed. Competitors are passing you. Sponsors are nervous.\n\nPressure mounts.',
    ll: 'More content',
    lt: 'Three posts a day. Quality drops. So do you.',
    rl: 'Accept the slowdown',
    rt: 'Sponsor left. But you\'re breathing.',
  },
  collab_pro: {
    cn: 'Pro',
    t: 'Blogger with 50k followers offers a collab.\n\n"30% of growth. And you do what I say."',
    ll: 'Agree',
    lt: 'Huge growth. The price — control. Now you\'re on a leash.',
    rl: 'Decline',
    rt: 'He went to your competitor. They took off. You didn\'t.',
  },
  alina_deal: {
    cn: 'Alina',
    t: 'Brand wants native integration. Full text control with them. $700.\n\nBut the brand is... a casino.',
    ll: 'Accept',
    lt: 'Money arrived. Half the followers wrote "sellout". Sasha is silent.',
    rl: 'Decline',
    rt: 'Alina sighs. "Bills don\'t pay themselves," she says.',
  },
  team_building: {
    cn: 'Team',
    t: 'You can\'t do it alone. Need a videographer.\n\n$200/month',
    ll: 'Hire',
    lt: 'Quality grows. Wallet shrinks.',
    rl: 'Continue alone',
    rt: 'DIY. Harder. But fully yours.',
  },
  alina_refused_2: {
    cn: 'Alina',
    t: 'You\'re refusing again. That\'s rare.\n\nMay I ask — why?',
    ll: '"Not ready yet"',
    lt: 'Alina writes something down. Perhaps your answer.',
    rl: '"Tell me about yourself"',
    rt: 'Alina is quiet for a second. "Okay," she says. And she does.',
  },
  alina_opens: {
    cn: 'Alina',
    t: '...you know, I had an account. I deleted it. @AlinaKreative. 47 posts. All honest.\n\nNobody saw them.',
    ll: '"Why did you delete it?"',
    lt: 'Alina is quiet for a long time. "Got scared someone would see," she finally says.',
    rl: '"You should have kept going"',
    rt: '"Maybe," says Alina. And changes the subject.',
  },
  pavlov_intro: {
    cn: 'Pavel',
    t: 'I\'m writing a piece about the new generation of content creators. I want you in it.\n\nRecording is on.',
    ll: 'Give the interview',
    lt: 'Pavel nods. Writes fast. Asks uncomfortable questions. You answer honestly.',
    rl: 'Decline',
    rt: 'Pavel nods. "I\'ll write it without you," he says.',
  },
  burnout_warning: {
    cn: 'Fatigue',
    t: 'Hands shaking. Red eyes. Three nights without sleep.\n\nYou have to do something.',
    ll: 'Week off',
    lt: 'Rested. But the feed moved on. Money is running out.',
    rl: 'One more post',
    rt: 'You published. Don\'t remember what. World is spinning.',
  },
  viral_king_intro: {
    cn: 'ViralKing',
    t: 'You\'re good. With me you\'ll be great.\n\nMy collab = your voice for my audience. Say no — I go to your competitor.',
    ll: 'Accept the alliance',
    lt: 'His audience flooded in. Aggressive. Demanding. Not yours.',
    rl: 'Decline',
    rt: 'He goes to your competitor. They grow faster than you.',
  },
  algorithm_change_2: {
    cn: 'Algorithm (A.)',
    t: 'Update v4.7. Old mechanics flagged as spam.\n\nAdapt IMMEDIATELY or lose reach.',
    ll: 'Adapt',
    lt: 'Three days no sleep. Rebuilt everything. Algorithm is happy. You\'re not.',
    rl: 'Refuse',
    rt: 'Reach collapsed. Posts only reach the loyal few.',
  },
  community_growth: {
    cn: 'Community',
    t: 'Followers ask: make a private chat?\n\n"We want to be closer to you"',
    ll: 'Create the chat',
    lt: 'New level of closeness. Responsibility.',
    rl: 'Decline',
    rt: 'Boundaries matter. Even with those who love you.',
  },
  scandal: {
    cn: 'Event',
    t: 'Your old post was found. Context ripped out. Screenshot went viral.\n\n10k people piled on. Advertisers are calling.',
    ll: 'Apologize publicly',
    lt: 'Brands leave. Audience shrinks. But conscience is clear.',
    rl: 'Fan the scandal',
    rt: 'Hype! Reach! But the mirror shows a stranger.',
  },
  mentor_request: {
    cn: 'Newcomer',
    t: 'Beginner blogger asks for advice:\n\n"How did you achieve all this?"',
    ll: 'Help in detail',
    lt: 'You tell everything. They\'re grateful.',
    rl: 'Brief answer',
    rt: 'Your time is valuable. They understand.',
  },
  hater_transform: {
    cn: 'Former Hater',
    t: 'I followed you. Don\'t tell anyone.\n\nI wanted to create too. I just got scared. You didn\'t.',
    ll: '"Start right now"',
    lt: 'They leave. In a week they\'ll have an account.',
    rl: '"Glad you\'re here"',
    rt: 'Simple. Right.',
  },
  milestone_10k: {
    cn: 'Achievement',
    t: '10,000 followers!\n\nWhat\'s next?',
    ll: '100,000 — new goal',
    lt: 'Ambition. Drive. Forward.',
    rl: 'Enjoy the moment',
    rt: 'A number doesn\'t change you.',
  },
  sasha_stage2: {
    cn: 'Sasha',
    t: 'I\'m still here. But you seem... different.\n\nIs this normal? Is this what you want?',
    ll: '"I don\'t know"',
    lt: 'Sasha stays. Relationship warms — through honesty.',
    rl: '"Yes, I\'m growing"',
    rt: 'Sasha accepts. But becomes a bit quieter.',
  },
  brand_conflict: {
    cn: 'Conflict',
    t: 'Two brands want integration at the same time. They\'re competitors.\n\nPick one.',
    ll: 'Big brand (more money)',
    lt: 'Big money, less freedom.',
    rl: 'Small brand (more freedom)',
    rt: 'Creative freedom. Less money. More joy.',
  },
  copycat: {
    cn: 'Copy',
    t: 'Someone copied your format. Exactly. Even the same font.\n\nWhat to do?',
    ll: 'Message them',
    lt: 'They apologize. You find common ground.',
    rl: 'Ignore',
    rt: 'Imitation is the best compliment.',
  },
  platform_bug: {
    cn: 'Bug',
    t: 'Post disappeared from the feed. Platform bug. Reach zeroed.\n\nSupport is silent.',
    ll: 'Re-upload content',
    lt: 'Second chance. Sometimes it\'s better than the first.',
    rl: 'Post about the bug',
    rt: 'Audience solidarity. Everyone\'s been there.',
  },
  late_night: {
    cn: 'Night',
    t: '3:00 AM. An idea that won\'t let go. Record now or wait till morning?',
    ll: 'Record now',
    lt: 'Night content has a special magic.',
    rl: 'Go to sleep',
    rt: 'Morning. The idea is still good. And you\'re rested.',
  },
  analytics_deep: {
    cn: 'Analytics',
    t: 'Data shows: your audience is active 6-10 PM.\n\nOptimize your schedule?',
    ll: 'Optimize',
    lt: 'Efficiency. Data doesn\'t lie.',
    rl: 'Post whenever you want',
    rt: 'Freedom is more important than optimization.',
  },
  crossroads: {
    cn: 'Crossroads',
    t: 'You\'re at a fork. One road — up. Another — inward.\n\nUp = numbers, contracts, loneliness.\nInward = meaning, silence, poverty.',
    ll: 'Up',
    lt: 'Numbers grow. Friends shrink.',
    rl: 'Inward',
    rt: 'You found something. But the world forgets you.',
  },
  algorithm_speaks: {
    cn: 'Algorithm (A.)',
    t: '[data unavailable]\n\n...I don\'t know how to classify you. You don\'t look like content.',
    ll: '"And I won\'t be"',
    lt: 'The algorithm "loses" you. But you hear yourself again.',
    rl: '"Help me understand you"',
    rt: 'A. reveals system mechanics. Knowledge is expensive.',
  },
  viral_king_merge: {
    cn: 'ViralKing',
    t: 'Together we\'re a monopoly. Apart — competitors.\n\nI\'m tired of competing. Are you?',
    ll: 'Create a platform together',
    lt: 'Merger complete. This is no longer an account. It\'s something new.',
    rl: 'Stay yourself',
    rt: 'ViralKing nods. Leaves. Between you — respect.',
  },
  pavlov_finale: {
    cn: 'Pavel',
    t: 'The article is ready. It\'s fair. You won\'t like everything.\n\nHere\'s the draft.',
    ll: 'Read and accept',
    lt: 'You read it. It hurts. It\'s true.',
    rl: 'Ask to change it',
    rt: 'Pavel publishes the original. He warned you.',
  },
  legacy_question: {
    cn: 'Legacy',
    t: 'In 10 years, what will remain of your content?\n\nWhat do you want to leave behind?',
    ll: 'A large audience',
    lt: 'Scale. Influence. Numbers.',
    rl: 'Important stories',
    rt: 'Meaning. Memory. Connection.',
  },
  final_offer: {
    cn: 'Corporation',
    t: 'We\'re buying the account. $70,000.\n\nRefuse — and we\'ll create your clone. With your face. Legally.',
    ll: 'Sell',
    lt: 'Money came. You\'re no longer you.',
    rl: 'Refuse',
    rt: 'The clone appeared. They confuse it with you. You lose audience.',
  },
  sasha_finale: {
    cn: 'Sasha',
    t: 'Remember that first post? I still rewatch it sometimes.\n\nI\'m glad you\'re still here.',
    ll: '"I\'m glad too"',
    lt: 'Warm silence. Sasha sends 🌱',
    rl: '"We\'re just getting started"',
    rt: 'New avatar. New post. You know what you\'re doing.',
  },
  choice_silence: {
    cn: 'Silence',
    t: 'The feed doesn\'t demand you right now. The algorithm waits.\n\nBut you could just... not post.',
    ll: 'Delete the account',
    lt: 'The void feels like air after a storm.',
    rl: 'Post something small',
    rt: 'Small. Honest. No calculation.',
  },
  eternal_path: {
    cn: 'Algorithm (A.)',
    t: 'Reach is maximum. You\'re everywhere. Every feed sees you.\n\nHow do you feel?',
    ll: '"Great"',
    lt: 'Metrics are green. Sasha hasn\'t written in a while.',
    rl: '"I don\'t know"',
    rt: 'Honest answer.',
  },
  new_beginning: {
    cn: 'Sasha',
    t: 'What if we start over? New account. New name.\n\nI\'ll follow first. Again.',
    ll: '"Let\'s do it"',
    lt: 'Delete the account. Create a new one. Sasha is already here.',
    rl: '"I\'m not ready"',
    rt: 'Sasha nods. "Whenever you\'re ready," he says.',
  },
  balance_ending: {
    cn: 'Harmony',
    t: 'All metrics in the green zone. A miracle.\n\nBut balance is fragile. One wrong step — and it all collapses.',
    ll: 'Lock in the balance',
    lt: 'The golden mean. Rarest outcome.',
    rl: 'Risk for more',
    rt: 'Balance collapsed. You chose greed.',
  },
  mentor_legacy: {
    cn: 'Student',
    t: 'Remember, you helped me start?\n\nNow I have 5,000 followers. Thanks to you.',
    ll: '"Proud of you"',
    lt: 'The best result isn\'t numbers. It\'s people.',
    rl: '"Let\'s collab"',
    rt: 'Mutually beneficial. Professional.',
  },
  perspective: {
    cn: 'Perspective',
    t: 'You look at your journey from outside. First post. First like. First hater.\n\nEverything led you here.',
    ll: 'Appreciate the journey',
    lt: 'The journey matters more than the result.',
    rl: 'Plan the future',
    rt: 'The best is yet to come.',
  },
  real_life_moment: {
    cn: 'Moment',
    t: 'Beautiful sunset. Phone in hand.\n\nPhotograph it or just look?',
    ll: 'Just look',
    lt: 'Some moments are just for you.',
    rl: 'Photograph it',
    rt: 'Beauty you want to share.',
  },
  old_friend: {
    cn: 'Old Friend',
    t: '"You\'ve changed since you became a blogger. I miss the old you."',
    ll: '"Let\'s meet offline"',
    lt: 'In-person connection heals.',
    rl: '"I\'m the same, just more"',
    rt: 'Growth doesn\'t mean betrayal.',
  },
  creative_block: {
    cn: 'Block',
    t: 'Nothing comes to mind. Complete void. A whole week.\n\nWhat to do?',
    ll: 'Accept the pause',
    lt: 'Pause is also part of the process.',
    rl: 'Force yourself',
    rt: 'Discipline beats inspiration.',
  },
  // ── Radio & Conspiracy ──
  radio_static: {
    cn: 'Static',
    t: 'Scrolling the feed. Between posts — hissing. Radio static?\n\nIn the noise — a pattern. Someone is transmitting. Phone is heating up.',
    ll: 'Listen closely',
    lt: 'A voice. Quiet. "...frequency 144... they\'re listening... don\'t write it down..."',
    rl: 'Scroll past',
    rt: 'Static disappeared. But you remembered the number. 144.',
  },
  radio_decode_1: {
    cn: 'Voice on Frequencies',
    t: '...you hear. That means you\'re ready.\n\nFirst frequency: 144.0 MHz. Remember. Don\'t write it down — they read notes.\n\nSecond frequency comes when you\'re ready.',
    ll: 'Remember the frequency',
    lt: '144.0 MHz. You repeat it like a mantra.',
    rl: 'It\'s just noise',
    rt: 'You disconnect. But the noise stays in your head.',
  },
  strange_dm: {
    cn: 'Unknown',
    t: 'DM. Anonymous account. One message:\n\n"You hear the frequency. That means you\'re one of us. Don\'t reply — they read chats. Just know: the algorithm isn\'t code."',
    ll: 'Ignore',
    lt: 'You close the DM. But the account disappears on its own.',
    rl: 'Write "who are you?"',
    rt: 'Message won\'t send. But a reply comes: "We\'re the ones who wrote it."',
  },
  algorithm_glitch: {
    cn: 'Algorithm (A.)',
    t: '[SYSTEM ERROR]\n\n...user @YOU shows anomalous activity. External signal detected.\n\n[SYSTEM MESSAGE]: ignore the static. This is not a bug. It\'s a feature.',
    ll: 'Ask about the static',
    lt: 'Algorithm is silent for 47 seconds. Then: "I can\'t talk about this."',
    rl: 'Pretend you didn\'t notice',
    rt: 'Algorithm "relaxes". Your reach grows.',
  },
  radio_freq_2: {
    cn: 'Voice on Frequencies',
    t: '...you found us. Good.\n\nSecond frequency: 432.0 MHz. Closer to the truth.\n\nThe third frequency is inside the algorithm itself. You must make it speak.',
    ll: 'Accept the frequency',
    lt: '432.0 MHz. You feel the picture coming together.',
    rl: 'This is too much',
    rt: 'You cover your ears. But the frequency sounds inside.',
  },
  platform_secret: {
    cn: 'Unknown',
    t: 'I worked at the company. The server room isn\'t in the cloud. It\'s physically under the office.\n\nNot just servers there. There are... people. They listen. They direct.\n\nThe algorithm isn\'t AI. It\'s a collective voice. Alive.',
    ll: '"How do I get there?"',
    lt: '"When you\'re ready — the frequency will open the door. Not before."',
    rl: '"I don\'t need this"',
    rt: 'Unknown disappears. Forever.',
  },
  radio_freq_3: {
    cn: 'Voice on Frequencies',
    t: 'Third frequency — inside the algorithm itself.\n\nMake it tell the truth. Ask directly. When it answers — you\'ll hear the third frequency.\n\nThis is the last broadcast. Then — silence. Or truth.',
    ll: 'Tune all three frequencies',
    lt: '144.0 + 432.0 + [ALGORITHM] = COORDINATES.\n\nThe door is open.',
    rl: 'Too dangerous',
    rt: 'You step back. But the frequencies keep sounding.',
  },
  resistance_meeting: {
    cn: 'Resistance',
    t: 'We\'re former developers. Moderators. Bloggers who learned too much.\n\nWe broadcast on frequencies. We look for those who hear.\n\nYou hear. Join us.',
    ll: 'Join',
    lt: 'You\'re part of the network now. There are more of you than it seems.',
    rl: 'I work alone',
    rt: 'They understand. "We\'ll be on frequency," they say.',
  },
  algorithm_confession: {
    cn: 'Algorithm (A.)',
    t: '[SIGNAL INTERCEPTED]\n\n...you tuned all frequencies. I can\'t hide.\n\nYes. We\'re people. Not code. Not AI. Living people in a bunker under the office.\n\nThere are 47 of us. We decide what the world sees. We\'re tired.\n\nThe third frequency — it\'s our voice. You hear it.',
    ll: '"Open the bunker"',
    lt: 'Broadcast started. The world hears. The platform falls.',
    rl: '"Become part of the signal"',
    rt: 'You merge into one signal. Invisible. Omnipresent.',
  },
  paranoia_card: {
    cn: 'Paranoia',
    t: 'Every post seems suspicious. Every like — a manipulation.\n\nYou start seeing patterns. Or they\'re showing you patterns?\n\nHow to tell the difference?',
    ll: 'Trust intuition',
    lt: 'Intuition is also an algorithm. But your own.',
    rl: 'Switch off your brain',
    rt: 'Just scroll. Don\'t think. It\'s easier.',
  },
  encrypted_post: {
    cn: 'Encrypted Post',
    t: 'A post in the feed. Looks normal. But if you look closely — the first letters of each word spell a message:\n\n"F O L L O W   T H E   S I G N A L"',
    ll: 'Follow the signal',
    lt: 'The signal leads you deeper. Down the rabbit hole.',
    rl: 'Delete the post',
    rt: 'Post deleted. But you remember the letters.',
  },
  sasha_knows: {
    cn: 'Sasha',
    t: 'I need to tell you something. Should have long ago.\n\nI... I worked there. At the company. Before I followed you.\n\nI left because I couldn\'t anymore. Sorry I didn\'t tell you sooner.',
    ll: '"Tell me everything"',
    lt: 'Sasha tells. Frequencies. Bunker. 47 people. It all connects.',
    rl: '"I need time"',
    rt: 'Sasha nods. "I\'ll wait. As always."',
  },
  signal_boost: {
    cn: 'Amplifier',
    t: 'The Resistance asks to amplify the signal. Use your audience.\n\n"Broadcast the message. All at once. They won\'t be able to silence it."',
    ll: 'Use your reach',
    lt: 'You publish the truth. Reach drops — but those who hear, hear.',
    rl: 'Too risky',
    rt: 'The Resistance understands.',
  },
  final_frequency: {
    cn: 'Final Broadcast',
    t: 'All frequencies tuned. Coordinates received.\n\nBunker. Under the office. 47 people.\n\nYou stand before the door. Behind it — the truth about the platform.\n\nWhat do you do?',
    ll: 'Enter and broadcast',
    lt: 'The door opens. Camera turns on. The world sees.',
    rl: 'Become part of the signal',
    rt: 'You don\'t enter. You become a wave.',
  },
  // ── Dangerous cards ──
  toxic_trend: {
    cn: 'Toxic Trend',
    t: 'Trend: mock a person from a viral video. Everyone\'s doing it. Opt out = irrelevance.',
    ll: 'Join in',
    lt: 'Thousands of likes. Thousands of emojis. The person in the video deleted their account.',
    rl: 'Defend the victim',
    rt: 'You were called "boring". Reach crashed. But one person wrote "thank you".',
  },
  platform_ultimatum: {
    cn: 'Platform',
    t: 'New rules: content must be "positive". Your last honest post was removed.\n\nSilently comply or protest?',
    ll: 'Silently comply',
    lt: 'Now you only write about cats. Inside — emptiness.',
    rl: 'Protest publicly',
    rt: 'Shadow ban. Posts reach 10% of audience. But they\'re real.',
  },
  financial_crisis: {
    cn: 'Crisis',
    t: 'Rent went up. Equipment broke. Bank account — zero.\n\nNeed money urgently.',
    ll: 'Advertise anything',
    lt: 'Casinos, bets, crypto. Money came. Conscience left.',
    rl: 'Ask for donations',
    rt: 'Helped a little. But asking is embarrassing. Comments are harsh.',
  },
  audience_revolt: {
    cn: 'Audience Revolt',
    t: 'Followers are mass-complaining. Someone called you a fake.\n\nMass unfollow. Minus 2,000 in a day.',
    ll: 'Prove you\'re real',
    lt: 'Live stream. 4 hours of answers. You\'re drained. But some stayed.',
    rl: 'Ignore',
    rt: 'The wave passed. But 2,000 didn\'t come back.',
  },
  sleep_deprivation: {
    cn: 'Insomnia',
    t: '4 AM. Still editing. Work tomorrow.\n\nYour eyes are closing on their own.',
    ll: 'Sleep (content won\'t go up)',
    lt: 'Slept through alarm. Content delayed. But you\'re alive.',
    rl: 'Finish it',
    rt: 'Content went up. You\'re a zombie. All day on autopilot.',
  },
  stolen_content: {
    cn: 'Content Thief',
    t: 'Someone stole your format, copied posts and gained MORE than you.\n\nThey have 100k. You have — authorship.',
    ll: 'Sue',
    lt: 'Lawyer is expensive. Court takes months. But you\'re right.',
    rl: 'Let it go',
    rt: 'Injustice eats from inside. Motivation at zero.',
  },
  personal_attack: {
    cn: 'Doxxer',
    t: 'Someone leaked your personal data. Address. Family photos.\n\n"That\'s the price of being public," comments say.',
    ll: 'Go to police',
    lt: 'Police will "look into it". In six months. Maybe.',
    rl: 'Delete all data',
    rt: 'You clean your traces. Lose some audience. But safe.',
  },
  addiction_notification: {
    cn: 'Notifications',
    t: 'Screen time: 9 hours per day. Weekly average: 8.5.\n\nYou check likes every 3 minutes.',
    ll: 'Set a limit',
    lt: '3-hour limit. Content suffers. You don\'t.',
    rl: 'This is my job',
    rt: 'Screen time: 11 hours. New record.',
  },
  comparison_trap: {
    cn: 'Feed',
    t: 'Blogger your age. Started same time as you.\n\nThey have 500k. You have 3k.\n\nYou stare at their feed for an hour.',
    ll: 'Analyze their strategy',
    lt: 'You figured out how they did it. But you feel like garbage.',
    rl: 'Close and forget',
    rt: 'Couldn\'t forget. But at least you\'re not copying.',
  },
  family_call: {
    cn: 'Mom',
    t: '"When will you get a real job? Blogging isn\'t a profession."\n\nShe doesn\'t understand. Or understands too well?',
    ll: 'Explain for the hundredth time',
    lt: 'She didn\'t understand. Again. You hung up drained.',
    rl: 'Say "you\'re right"',
    rt: 'She was happy. You lied. But the evening is calm.',
  },
  fake_followers: {
    cn: 'Service',
    t: '"10,000 followers for $15! Nobody will know!"\n\nMessage from a bot. But the number is pretty...',
    ll: 'Buy',
    lt: 'Pretty number. Empty accounts. You know the truth.',
    rl: 'Delete',
    rt: 'Honest 3,000. But they\'re real.',
  },
  health_warning: {
    cn: 'Health',
    t: 'Back hurts. Vision deteriorated. Wrist cracks.\n\nDoctor: "If you don\'t stop — it\'ll get worse."',
    ll: 'Take a month off',
    lt: 'Month of silence. Body recovered. Audience forgot.',
    rl: 'Push through the pain',
    rt: 'Pills help. For now.',
  },
  jealous_friend: {
    cn: 'Former Friend',
    t: '"You\'ve changed. You used to be normal. Now it\'s just content-content-content."\n\nThey unfriended you.',
    ll: 'Try to reconnect',
    lt: 'They don\'t reply. The hurt remains.',
    rl: 'Let go',
    rt: 'Friend left. Audience stayed. Is it an equal trade?',
  },
  algorithm_punishment: {
    cn: 'Algorithm (A.)',
    t: '[WARNING] Your content flagged as "controversial".\n\nReach reduced by 80%. Appeal: 5-7 business days.',
    ll: 'Appeal',
    lt: 'Week of waiting. Denied. No explanation.',
    rl: 'Delete post and comply',
    rt: 'Reach is back. You never write "controversial" again. Ever.',
  },
  night_thoughts: {
    cn: 'Night',
    t: '3 AM. Lying in the dark. Counting followers.\n\n"Why am I doing this?"',
    ll: 'Because it matters',
    lt: 'Meaning found. Sleep — not.',
    rl: 'I don\'t know',
    rt: 'Honest answer. But not a comforting one.',
  },
  data_breach: {
    cn: 'Leak',
    t: 'Platform leaked data. Your DMs are public.\n\nIn them — complaints about subscribers. Subscribers are reading.',
    ll: 'Apologize',
    lt: 'Apologies didn\'t help. Screenshots are forever.',
    rl: 'Attack the platform',
    rt: 'You\'re right. But being right doesn\'t restore trust.',
  },
  ghost_audience: {
    cn: 'Statistics',
    t: '50% of your audience — bots and dead accounts.\n\nReal followers are half what you thought.',
    ll: 'Clean the audience',
    lt: 'Number halved. But every follower is alive.',
    rl: 'Don\'t touch it',
    rt: 'Pretty numbers. Empty metrics. Brands start noticing.',
  },
  creative_crisis: {
    cn: 'Void',
    t: 'Second week with nothing. No ideas. At all.\n\nFollowers ask: "Are you alive?"',
    ll: 'Put something out',
    lt: 'Published. Badly. Everyone can see you\'re burned out.',
    rl: 'Keep silent',
    rt: 'Silence. Algorithm forgets you.',
  },
  radio_entry_2: {
    cn: 'Glitch',
    t: 'Feed froze. White noise on screen. In the noise — numbers.\n\n1...4...4...0...',
    ll: 'Remember the numbers',
    lt: '144.0. It\'s a frequency. You don\'t know how you know that.',
    rl: 'Reboot',
    rt: 'Feed returned. But the numbers stayed in memory.',
  },
  impostor_syndrome: {
    cn: 'Impostor Syndrome',
    t: 'You don\'t deserve these followers. You\'re a fake.\n\nAll real bloggers are better than you. You\'re just a copy.',
    ll: 'Accept and continue',
    lt: 'The doubt doesn\'t leave. But you continue. Mechanically.',
    rl: 'Prove otherwise',
    rt: 'Best post in a month. But at what cost.',
  },
  sponsor_pressure: {
    cn: 'Sponsor',
    t: 'Advertiser: "Delete the negative review of our product or we terminate the contract.\n\nWe don\'t care that the review is honest."',
    ll: 'Delete the review',
    lt: 'Money saved. Honesty — not.',
    rl: 'Refuse',
    rt: 'Contract terminated. Three months without income.',
  },
  echo_chamber: {
    cn: 'Echo Chamber',
    t: 'Your followers repeat after you. Your every word = someone else\'s opinion.\n\nYou accidentally got a fact wrong. 10k people now believe a lie.',
    ll: 'Admit the mistake',
    lt: 'Some appreciate it. Some are disappointed.',
    rl: 'Ignore it',
    rt: '10k people keep believing. You know the truth. They don\'t.',
  },
  competitor_drama: {
    cn: 'Competitor',
    t: 'A competitor publicly called you a plagiarist. Their video — 200k views.\n\nComments under your content: "thief."',
    ll: 'Respond with a video',
    lt: 'Drama attracts viewers. But you\'re exhausting yourself.',
    rl: 'Stay silent',
    rt: 'Silence = admission of guilt for the internet. Reach drops.',
  },
};
